...
int bonnie_main(int argc, char **argv)
{
    ...
    for (next = 0; next < SeekProcCount; next++) {	/* for each seek proc */
        if ((child = fork()) == -1)
            ...
            if (write(seek_feedback[1], seeker_report, sizeof(seeker_report)) != sizeof(seeker_report))
                io_error("pipe write");
            exit(0);
        }			/* child process */
    }				/* for each seek proc */
...
{
    printf("              ");
    printf("-------Sequential Output-------- ---Sequential Input-- --Random--\n");
    printf("              ");
    printf("-Per Char- --Block--- -Rewrite-- -Per Char- --Block--- --Seeks---\n");
    printf("Machine    MB ");
...
static void file_read_rewrite_block()
{
    timestamp();
    bufindex = 0;
    if ((words = file_read(name, 0, (char *) buf, Chunk)) == -1)
        io_error("rewrite read");

        if (file_write(name, words, (char *) buf, words) == -1)
        io_error("re write(2)");
    if ((words = file_read(name, words, (char *) buf, Chunk)) == -1)
