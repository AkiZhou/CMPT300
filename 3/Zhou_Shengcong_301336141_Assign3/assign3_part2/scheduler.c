/*
 * Reference: https://github.com/hungpchu/Scheduler-with-Signals/tree/master/Code
 */
...
static void init_sched_queue(int queue_size)
{
	...
	struct sigevent *evPtr = malloc(sizeof(struct sigevent));
	evPtr->sigev_notify = SIGEV_SIGNAL;
	evPtr->sigev_signo = SIGALRM;
	if (timer_create(CLOCK_REALTIME, evPtr, &timer)!=0) {
...
void setup_sig_handlers() {
	...
	struct sigaction *act_timer = (struct sigaction*)malloc(sizeof(struct sigaction));
